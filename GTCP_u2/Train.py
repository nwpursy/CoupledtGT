...
from GTCP_u2.EvaluationMetrics import Evaluation_Utils
...
def caseStudy(total_input,total_truth,total_my,total_coupling,total_sigma,total_q,roll):
    ...
    steps=[]
    names = []
    for i in range(Const.target_time_window):
        steps.append('step {}'.format(i+1))
    for i in range(Const.node_count):
        names.append('node {}'.format(i))
    for case in roll:
        title='case '+str(case)
        ...
        util.plotManyHeatmap(4,3,coupling,title,steps)
        sigma=total_sigma[case]
        q=total_q[case]
        util.plotOneHeatmap(sigma,title+' :sigma',xticklabels=['x','y'],yticklabels=steps)
        util.plotOneHeatmap(q, title + ' :q',xticklabels=names,yticklabels=steps,x_rot=270)

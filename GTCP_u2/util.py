...
def plotOneHeatmap(data,save_path='',title='',figsize=(6.4,4.8),xticklabels=None,yticklabels=None,x_rot=0):
    fig,ax=plt.subplots(figsize=figsize)
    plt.title(title)
    # plt.title('Ubuntu 2 '+title)
    im=plt.imshow(data,cmap=plt.cm.hot)
    # if not xticklabels==None:
    #     plt.xticks(range(data.size(1)),xticklabels,rotation=x_rot)
    # if not yticklabels==None:
    #     plt.yticks(range(len(data)),yticklabels)
    plt.colorbar(orientation='vertical')
    # if save_path!='':
    #     plt.savefig(save_path)
    plt.show()
def plotManyHeatmap(n_row,n_col,data_group,title,names,save_path=''):
    fig, ax = plt.subplots(n_row, n_col, sharex=True, sharey=True, constrained_layout=False, figsize=(5, 7))
    # fig.suptitle(title)
    fig.suptitle('Ubuntu 2 '+title)
    # plt.grid(linestyle='-.')
    # plt.legend()
    k = 0
    for i in range(n_row):
...
if __name__=='__main__':
    ...
    # plotOneHeatmap(a,save_path='./test.png',figsize=(2.5,5),xticklabels=['x','y'],yticklabels=names)
    # plotOneHeatmap(b,save_path='./test.png',figsize=(9,5),xticklabels=nodes,yticklabels=names,x_rot=270)

    plotManyHeatmap(4,3,a,'test',names,save_path='./test.png')

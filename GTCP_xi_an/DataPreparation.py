...
def readCSVFiles(pollution_csv_name,weather_csv_name,stations_csv_name,parentPath='../resource/data/xi_an'):
...
class DataAccess:
    ...
    '''
    def divideInoutSequences(self, dev_ratio, test_ratio, parentPath='../resource/preprocess/xi_an'):
...
def make_data(dataset_name,category,parentPath='../resource/preprocess/xi_an',shuffle=False,sample_ratio=1):
...
def process_coupled_locations(dataset_name,category,parentPath='../resource/preprocess/xi_an'
                                    ,outName='coupledGEO_XY_vectors',outPath='../resource/preprocess/xi_an'):
    data_dict = torch.load('{}/{}/{}set@in{}&out{}&step{}&offset{}.pt'.format(parentPath, dataset_name,
...
def load_locations(dataset_name,file_name='coupled_location',parent_path='../resource/preprocess/xi_an'):
    t=torch.load('{}/{}/{}.pt'.format(parent_path,dataset_name,file_name),map_location=Const.device)
    return t
def process_locations(dataset_name,category,parentPath='../resource/preprocess/xi_an'):
    data_dict = torch.load('{}/{}/{}set@in{}&out{}&step{}&offset{}.pt'.format(parentPath, dataset_name,
                                                                              category, Const.input_time_window,
                                                                              Const.target_time_window,
                                                                              Const.seq_sample_step, Const.step_offset))
...
if __name__=='__main__':
    pollution_df,weather_df,selected_stations=readCSVFiles(Const.dataset_name,
                                                           'xi_an2014-2022_processed','stations')
    ...
    t=load_locations(Const.dataset_name,file_name='points_set')
    print(t)
    process_coupled_locations(Const.dataset_name,'test')
    t=load_locations(Const.dataset_name,file_name='coupled_location')
    # print(t)
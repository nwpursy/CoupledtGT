...
class GaussianDiff2dPG(nn.Module):
    ...
    def forward(self,factors_batch):
        scaled_f1=util.min_max_scale(factors_batch[:,0],-100,300)
        scaled_f2=util.min_max_scale(factors_batch[:,1],-200,200)
        # scaled_f3=util.min_max_scale(factors_batch[:,2],10000,10400)
        scaled_factors=torch.stack([scaled_f1,scaled_f2],dim=-1)
